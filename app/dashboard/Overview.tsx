...
export default function Overview({
    ...
    const getMockDifficulty = (id: string, diff?: string) => {
        if (diff) {
            if (diff === 'Hard') return { label: 'Hard', color: 'text-red-400 border-red-500/40 bg-red-500/10' }
            if (diff === 'Medium') return { label: 'Medium', color: 'text-orange-400 border-orange-400/40 bg-orange-400/10' }
    ...
    // Filter pending suggestions for today
    const pendingTasks = useMemo(() => {
        return suggestions.filter(s => s.status === 'pending' || s.status === 'rolled_over')
    }, [suggestions])
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <span className="text-[10px] text-[#444] font-mono">{pendingTasks.length} tasks remaining</span>
                    </div>

                    <div className="space-y-2.5">
                        {pendingTasks.length > 0 ? pendingTasks.map(p => {
                            const diff = getMockDifficulty(p.problem_id, p.difficulty)
                            const isReview = p.is_review
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {isReview && (
                                                <span className="text-[8px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-1.5 py-0.5 rounded font-mono uppercase tracking-tighter">
                                                    Review
                                                </span>
                                            )}
                                        </div>
                                        <div className="flex items-center gap-2 mt-1">
                                            ...
                                            {isReview && (
                                                <span className="text-[9px] text-[#333] font-mono">
                                                    {dueProblems.find(dp => dp.problem_id === p.problem_id)?.retrievability || 0}% retention
                                                </span>
                                            )}
                                        </div>
                                    </div>
